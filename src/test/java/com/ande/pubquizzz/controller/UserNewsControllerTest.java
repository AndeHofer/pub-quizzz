...
class UserNewsControllerTest {
    ...
    void getLatestNews_authenticated_returnsLatestThreeByDefault() throws Exception {
        when(newsService.getLatestNews(3)).thenReturn(List.of(
                new NewsDTO(3L, "Neu 1", "Text", Instant.parse("2026-06-05T10:15:30Z"), true),
                new NewsDTO(2L, "Neu 2", "Text", Instant.parse("2026-06-04T10:15:30Z"), true)
        ));
    ...
    @Test
    void getLatestNews_unauthenticated_returnsJson401() throws Exception {
        mockMvc.perform(get("/api/news"))
