...
function renderLeaderboard(entries: AverageLeaderboardEntry[]): void {
    ...
    tbody.innerHTML = entries.map(e => `
        <tr class="border-b border-gray-200 hover:bg-gray-50">
            ...
            <td class="py-3 px-4 font-medium"><a href="/team.html?team=${encodeURIComponent(e.teamName)}&source=average" class="text-blue-600 hover:underline">${escapeHtml(e.teamName)}</a></td>
            <td class="py-3 px-4 text-center font-bold text-blue-700">${e.averagePoints.toFixed(2)}</td>
...
async function loadLeaderboard(): Promise<void> {
    ...
    try {
        const response = await fetch('/api/leaderboard/average');
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}`);
        }
        const entries: AverageLeaderboardEntry[] = await response.json();
